...
export default class Engine {
	...
	availableMoves(currentPlayer: Player): Node[] {
		...
		const availableMoves = new Set(currentPlayer.location.nextNonEmptyDescendants())
		...
		for (let distance = 2; distance <= moveDistance; ++distance) {
			let movesOfCurrentDistance: Node[] = lastMoves.reduce((moves, currentMove) => {
				...
				return !moves ? currentMove.nextNonEmptyDescendants() : moves.concat(currentMove.nextNonEmptyDescendants())
			}, new Array<Node>())
